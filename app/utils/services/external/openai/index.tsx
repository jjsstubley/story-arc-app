...
import { getMovieBySearchQuery } from "../tmdb/search";
  ...
  try {
    ...
    const enrichedData = {
      suggestions: await Promise.all(
        data.suggestions.map(async (movie: { title: string; year: string }) => {
    ...
    console.log('enrichedMovies', enrichedData)
    return json({ result: enrichedData });
  } catch (error) {
    console.error(error);
