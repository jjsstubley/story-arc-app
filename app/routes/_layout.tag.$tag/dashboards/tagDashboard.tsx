...
interface MoviesByTagInterface {
  tag: GenreInterface,
  movies: MovieListsInterface
}

export default function TagDashboard({ tagList }: {tagList: MoviesByTagInterface}) {
  return (
    <>
        <Box as="section" display="grid" gap={8} gridColumn={1} flex="1" p={4} pt={0} overflow="hidden">
            <Box as="section" flex="1" p={4} pt={0} overflow="hidden">
              <Box display="flex" justifyContent="space-between">
                <Heading as="h3" pb={4}>{tagList.tag.name}</Heading>
              </Box>
              <SimpleGrid
                ...
                >
                {
                  tagList.movies.results.map((item, i) => (
